...
formComponents.forEach(function (component) {
    ...
    form.addEventListener('submit', function (event) {
        ...
        function check_missing_infos(){

            const all_inputs = document.querySelector('.dragArea.row').querySelectorAll('input, select, textarea')       

            for (let i = 0; i < all_inputs.length; i++) {
                val = all_inputs[i].value.trim() || all_inputs[i].innerText
                indice = all_inputs[i].getAttribute('placeholder')
                if(val === ''){
                    all_inputs[i].focus()
                    return '❌ Merci de remplir tous les champs du formulaire, notamment le champ «'+indice+'».';
                }
            }
            return ''
        ...
        var error_all = check_missing_infos()
        //console.log({error_all})

        if(error_all.length === 0){
            Promise
